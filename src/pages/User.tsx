...
import { getAccessToken, getUserInfo, getLikes, getExercises, getChannel } from 'utility/api';
import { APIGetUserInfoData, ExerciseView, APIGetExerciseData } from 'utility/types';
...
function Userpage({ match } : UserpageProps) {
    ...
    useEffect(() => {
      ...
      getChannel(nickname).then((exerciseData) => {
        if (!ok) return;
        setExercises(exerciseData.map((data) => ({
          id: data.exercise_id,
          name: data.title,
